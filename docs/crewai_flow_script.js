...
class NodeRenderer {
  ...
  drawNodeShape(ctx, x, y, width, height, scale, nodeStyle, opacity = 1.0, node = null) {
    ...
    const isDarkMode = document.documentElement.getAttribute('data-theme') === 'dark';
    const nodeData = {"classify_input": {"type": "start", "method_signature": {"operationId": "classify_input", "parameters": {}, "returns": {"type": "null"}}, "source_code": "    @start()\n    def classify_input(self) -> None:\n        result = ClassificationCrew(\n            verbose=self._verbose,\n            guardrail_max_retries=self._guardrail_max_retries,\n        ).crew().kickoff(inputs={\n            \"user_input\": self.state.raw_input,\n            \"output_options\": \"/\".join(DocumentType),\n        })\n        self.state.input_type = result.raw\n", "source_lines": ["    @start()\n", "    def classify_input(self) -> None:\n", "        result = ClassificationCrew(\n", "            verbose=self._verbose,\n", "            guardrail_max_retries=self._guardrail_max_retries,\n", "        ).crew().kickoff(inputs={\n", "            \"user_input\": self.state.raw_input,\n", "            \"output_options\": \"/\".join(DocumentType),\n", "        })\n", "        self.state.input_type = result.raw\n"], "source_start_line": 42, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "extract_metadata": {"type": "listen", "trigger_methods": ["cv_or_job"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "extract_metadata", "parameters": {}, "returns": {}}, "source_code": "    @listen(\"cv_or_job\")\n    def extract_metadata(self) -> Any:\n        if self.state.input_type == DocumentType.CV:\n            # Extract cv metadata\n            metadata = CVMetadataExtractorCrew(\n                guardrail_max_retries=self._guardrail_max_retries,\n                verbose=self._verbose,\n                human_input=False,\n            ).crew().kickoff(inputs={\n                \"content\": self.state.raw_input,\n                \"educationlevel_options\": \"/\".join(EducationLevel),\n                \"experiencelevel_options\": \"/\".join(ExperienceLevel),\n            })\n        else:\n            # Extract job metadata\n            metadata = JobMetadataExtractorCrew(\n                guardrail_max_retries=self._guardrail_max_retries,\n                verbose=self._verbose,\n                human_input=False,\n            ).crew().kickoff(inputs={\n                \"content\": self.state.raw_input,\n                \"employmenttype_options\": \"/\".join(EmploymentType),\n                \"experiencelevel_options\": \"/\".join(ExperienceLevel),\n            })\n        self.state.metadata = json.loads(metadata.raw)\n", "source_lines": ["    @listen(\"cv_or_job\")\n", "    def extract_metadata(self) -> Any:\n", "        if self.state.input_type == DocumentType.CV:\n", "            # Extract cv metadata\n", "            metadata = CVMetadataExtractorCrew(\n", "                guardrail_max_retries=self._guardrail_max_retries,\n", "                verbose=self._verbose,\n", "                human_input=False,\n", "            ).crew().kickoff(inputs={\n", "                \"content\": self.state.raw_input,\n", "                \"educationlevel_options\": \"/\".join(EducationLevel),\n", "                \"experiencelevel_options\": \"/\".join(ExperienceLevel),\n", "            })\n", "        else:\n", "            # Extract job metadata\n", "            metadata = JobMetadataExtractorCrew(\n", "                guardrail_max_retries=self._guardrail_max_retries,\n", "                verbose=self._verbose,\n", "                human_input=False,\n", "            ).crew().kickoff(inputs={\n", "                \"content\": self.state.raw_input,\n", "                \"employmenttype_options\": \"/\".join(EmploymentType),\n", "                \"experiencelevel_options\": \"/\".join(ExperienceLevel),\n", "            })\n", "        self.state.metadata = json.loads(metadata.raw)\n"], "source_start_line": 60, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "handle_other": {"type": "listen", "trigger_methods": ["route_other"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "handle_other", "parameters": {}, "returns": {"type": "null"}}, "source_code": "    @listen(\"route_other\")\n    def handle_other(self) -> None:\n        msg = f\"Invalid document type. Expected '{DocumentType.CV}' or '{DocumentType.JOB}'. \" \\\n            \"Please, start a new evaluation.\"\n        logger.warning(msg)\n        return msg\n", "source_lines": ["    @listen(\"route_other\")\n", "    def handle_other(self) -> None:\n", "        msg = f\"Invalid document type. Expected '{DocumentType.CV}' or '{DocumentType.JOB}'. \" \\\n", "            \"Please, start a new evaluation.\"\n", "        logger.warning(msg)\n", "        return msg\n"], "source_start_line": 150, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "process_cv": {"type": "listen", "trigger_methods": ["route_cv"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "process_cv", "parameters": {}, "returns": {"type": "null"}}, "source_code": "    @listen(\"route_cv\")\n    def process_cv(self) -> None:\n        # Send info to CVToJobCrew\n        cv_crew = CVToJobCrew(\n            verbose=self._verbose,\n            guardrail_max_retries=self._guardrail_max_retries,\n        )\n        _ = cv_crew.crew().kickoff(\n            inputs={\"structured_cv\": self.state.metadata,\n                    \"related_jobs\": self.state.related_docs}\n        )\n        self.state.process_crew = cv_crew\n", "source_lines": ["    @listen(\"route_cv\")\n", "    def process_cv(self) -> None:\n", "        # Send info to CVToJobCrew\n", "        cv_crew = CVToJobCrew(\n", "            verbose=self._verbose,\n", "            guardrail_max_retries=self._guardrail_max_retries,\n", "        )\n", "        _ = cv_crew.crew().kickoff(\n", "            inputs={\"structured_cv\": self.state.metadata,\n", "                    \"related_jobs\": self.state.related_docs}\n", "        )\n", "        self.state.process_crew = cv_crew\n"], "source_start_line": 109, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "process_job": {"type": "listen", "trigger_methods": ["route_job"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "process_job", "parameters": {}, "returns": {"type": "null"}}, "source_code": "    @listen(\"route_job\")\n    def process_job(self) -> None:\n        # Send info to JobToCVCrew\n        job_crew = JobToCVCrew(\n            verbose=self._verbose,\n            guardrail_max_retries=self._guardrail_max_retries,\n        )\n        _ = job_crew.crew().kickoff(\n            inputs={\"structured_job\": self.state.metadata,\n                    \"related_cvs\": self.state.related_docs}\n        )\n        self.state.process_crew = job_crew\n", "source_lines": ["    @listen(\"route_job\")\n", "    def process_job(self) -> None:\n", "        # Send info to JobToCVCrew\n", "        job_crew = JobToCVCrew(\n", "            verbose=self._verbose,\n", "            guardrail_max_retries=self._guardrail_max_retries,\n", "        )\n", "        _ = job_crew.crew().kickoff(\n", "            inputs={\"structured_job\": self.state.metadata,\n", "                    \"related_cvs\": self.state.related_docs}\n", "        )\n", "        self.state.process_crew = job_crew\n"], "source_start_line": 122, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "query_to_db": {"type": "listen", "trigger_methods": ["extract_metadata"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "query_to_db", "parameters": {}, "returns": {}}, "source_code": "    @listen(extract_metadata)\n    def query_to_db(self) -> Any:\n        if self.state.input_type == DocumentType.CV:\n            collection_name = \"jobs\"\n        else:\n            collection_name = \"cvs\"\n\n        # Get matches from collection\n        related_docs = query_to_collection(\n            collection_name=collection_name,\n            query_text=self.state.raw_input,\n            country=self.state.metadata.get(\"country\"),\n            top_k=3,\n        )\n        self.state.related_docs = related_docs\n", "source_lines": ["    @listen(extract_metadata)\n", "    def query_to_db(self) -> Any:\n", "        if self.state.input_type == DocumentType.CV:\n", "            collection_name = \"jobs\"\n", "        else:\n", "            collection_name = \"cvs\"\n", "\n", "        # Get matches from collection\n", "        related_docs = query_to_collection(\n", "            collection_name=collection_name,\n", "            query_text=self.state.raw_input,\n", "            country=self.state.metadata.get(\"country\"),\n", "            top_k=3,\n", "        )\n", "        self.state.related_docs = related_docs\n"], "source_start_line": 86, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "render_and_export_report": {"type": "listen", "trigger_condition_type": "OR", "condition_type": "OR", "trigger_condition": {"type": "OR", "conditions": ["process_cv", "process_job"]}, "trigger_methods": ["process_cv", "process_job"], "method_signature": {"operationId": "render_and_export_report", "parameters": {}, "returns": {"type": "string"}}, "source_code": "    @listen(or_(process_cv, process_job))\n    def render_and_export_report(self) -> str:\n        # Markdown Report generation\n        report = render_to_markdown(\n            process_type=self.state.input_type,\n            metadata_dict=self.state.metadata,\n            related_docs=self.state.related_docs,\n            gap_analysis_output=self.state.process_crew.identify_gaps_task().output.json_dict,\n            inverview_questions_output=self.state.process_crew.generate_interview_questions_task().output.json_dict,\n        )\n\n        # Write the file\n        REPORT_OUTPUT_PATH.write_text(report, encoding=\"utf-8\")\n        return report\n", "source_lines": ["    @listen(or_(process_cv, process_job))\n", "    def render_and_export_report(self) -> str:\n", "        # Markdown Report generation\n", "        report = render_to_markdown(\n", "            process_type=self.state.input_type,\n", "            metadata_dict=self.state.metadata,\n", "            related_docs=self.state.related_docs,\n", "            gap_analysis_output=self.state.process_crew.identify_gaps_task().output.json_dict,\n", "            inverview_questions_output=self.state.process_crew.generate_interview_questions_task().output.json_dict,\n", "        )\n", "\n", "        # Write the file\n", "        REPORT_OUTPUT_PATH.write_text(report, encoding=\"utf-8\")\n", "        return report\n"], "source_start_line": 135, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "route_by_type_1": {"type": "router", "is_router": true, "router_paths": ["route_other", "cv_or_job"], "trigger_methods": ["classify_input"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "route_by_type_1", "parameters": {}, "returns": {"type": "string"}}, "source_code": "    @router(classify_input)\n    def route_by_type_1(self) -> str:\n        if (self.state.input_type == DocumentType.CV) or (self.state.input_type == DocumentType.JOB):\n            return \"cv_or_job\"\n        else:\n            return \"route_other\"\n", "source_lines": ["    @router(classify_input)\n", "    def route_by_type_1(self) -> str:\n", "        if (self.state.input_type == DocumentType.CV) or (self.state.input_type == DocumentType.JOB):\n", "            return \"cv_or_job\"\n", "        else:\n", "            return \"route_other\"\n"], "source_start_line": 53, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "route_by_type_2": {"type": "router", "is_router": true, "router_paths": ["route_job", "route_cv"], "trigger_methods": ["query_to_db"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "route_by_type_2", "parameters": {}, "returns": {"type": "string"}}, "source_code": "    @router(query_to_db)\n    def route_by_type_2(self) -> str:\n        if self.state.input_type == DocumentType.CV:\n            return \"route_cv\"\n        else:\n            return \"route_job\"\n", "source_lines": ["    @router(query_to_db)\n", "    def route_by_type_2(self) -> str:\n", "        if self.state.input_type == DocumentType.CV:\n", "            return \"route_cv\"\n", "        else:\n", "            return \"route_job\"\n"], "source_start_line": 102, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}};
...
class NetworkManager {
  ...
  async initialize() {
    try {
      ...
      this.nodes = new vis.DataSet([{"id": "classify_input", "label": "classify_input", "title": "\n            <div style=\"border-bottom: 1px solid rgba(102,102,102,0.15); padding-bottom: 8px; margin-bottom: 10px;\">\n                <div style=\"font-size: 13px; font-weight: 700; color: #333333; margin-bottom: 6px;\">classify_input</div>\n                <span style=\"display: inline-block; background: #FF5A50; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;\">start</span>\n            </div>\n        ", "shape": "custom", "size": 30, "level": 0, "nodeStyle": {"name": "classify_input", "bgColor": "var(--node-bg-start)", "borderColor": "var(--node-border-start)", "borderWidth": 3, "fontColor": "var(--node-text-color)"}, "opacity": 1.0, "glowSize": 0, "glowColor": null}, {"id": "extract_metadata", "label": "extract_metadata", "title": "\n            <div style=\"border-bottom: 1px solid rgba(102,102,102,0.15); padding-bottom: 8px; margin-bottom: 10px;\">\n                <div style=\"font-size: 13px; font-weight: 700; color: #333333; margin-bottom: 6px;\">extract_metadata</div>\n                <span style=\"display: inline-block; background: #333333; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;\">listen</span>\n            </div>\n        \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 3px; font-weight: 600;\">Condition</div>\n                    <span style=\"display: inline-block; background: rgba(102,102,102,0.12); color: #666666; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: 700;\">OR</span>\n                </div>\n            \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 4px; font-weight: 600;\">Triggers</div>\n                    <ul style=\"list-style: none; padding: 0; margin: 0;\"><li style=\"margin: 3px 0;\"><code style=\"background: rgba(102,102,102,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #333333; border: 1px solid rgba(102,102,102,0.12);\">cv_or_job</code></li></ul>\n                </div>\n            ", "shape": "custom", "size": 30, "level": 2, "nodeStyle": {"name": "extract_metadata", "bgColor": "var(--node-bg-listen)", "borderColor": "var(--node-border-listen)", "borderWidth": 3, "fontColor": "var(--node-text-color)"}, "opacity": 1.0, "glowSize": 0, "glowColor": null}, {"id": "handle_other", "label": "handle_other", "title": "\n            <div style=\"border-bottom: 1px solid rgba(102,102,102,0.15); padding-bottom: 8px; margin-bottom: 10px;\">\n                <div style=\"font-size: 13px; font-weight: 700; color: #333333; margin-bottom: 6px;\">handle_other</div>\n                <span style=\"display: inline-block; background: #333333; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;\">listen</span>\n            </div>\n        \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 3px; font-weight: 600;\">Condition</div>\n                    <span style=\"display: inline-block; background: rgba(102,102,102,0.12); color: #666666; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: 700;\">OR</span>\n                </div>\n            \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 4px; font-weight: 600;\">Triggers</div>\n                    <ul style=\"list-style: none; padding: 0; margin: 0;\"><li style=\"margin: 3px 0;\"><code style=\"background: rgba(102,102,102,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #333333; border: 1px solid rgba(102,102,102,0.12);\">route_other</code></li></ul>\n                </div>\n            ", "shape": "custom", "size": 30, "level": 2, "nodeStyle": {"name": "handle_other", "bgColor": "var(--node-bg-listen)", "borderColor": "var(--node-border-listen)", "borderWidth": 3, "fontColor": "var(--node-text-color)"}, "opacity": 1.0, "glowSize": 0, "glowColor": null}, {"id": "process_cv", "label": "process_cv", "title": "\n            <div style=\"border-bottom: 1px solid rgba(102,102,102,0.15); padding-bottom: 8px; margin-bottom: 10px;\">\n                <div style=\"font-size: 13px; font-weight: 700; color: #333333; margin-bottom: 6px;\">process_cv</div>\n                <span style=\"display: inline-block; background: #333333; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;\">listen</span>\n            </div>\n        \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 3px; font-weight: 600;\">Condition</div>\n                    <span style=\"display: inline-block; background: rgba(102,102,102,0.12); color: #666666; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: 700;\">OR</span>\n                </div>\n            \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 4px; font-weight: 600;\">Triggers</div>\n                    <ul style=\"list-style: none; padding: 0; margin: 0;\"><li style=\"margin: 3px 0;\"><code style=\"background: rgba(102,102,102,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #333333; border: 1px solid rgba(102,102,102,0.12);\">route_cv</code></li></ul>\n                </div>\n            ", "shape": "custom", "size": 30, "level": 5, "nodeStyle": {"name": "process_cv", "bgColor": "var(--node-bg-listen)", "borderColor": "var(--node-border-listen)", "borderWidth": 3, "fontColor": "var(--node-text-color)"}, "opacity": 1.0, "glowSize": 0, "glowColor": null}, {"id": "process_job", "label": "process_job", "title": "\n            <div style=\"border-bottom: 1px solid rgba(102,102,102,0.15); padding-bottom: 8px; margin-bottom: 10px;\">\n                <div style=\"font-size: 13px; font-weight: 700; color: #333333; margin-bottom: 6px;\">process_job</div>\n                <span style=\"display: inline-block; background: #333333; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;\">listen</span>\n            </div>\n        \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 3px; font-weight: 600;\">Condition</div>\n                    <span style=\"display: inline-block; background: rgba(102,102,102,0.12); color: #666666; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: 700;\">OR</span>\n                </div>\n            \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 4px; font-weight: 600;\">Triggers</div>\n                    <ul style=\"list-style: none; padding: 0; margin: 0;\"><li style=\"margin: 3px 0;\"><code style=\"background: rgba(102,102,102,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #333333; border: 1px solid rgba(102,102,102,0.12);\">route_job</code></li></ul>\n                </div>\n            ", "shape": "custom", "size": 30, "level": 5, "nodeStyle": {"name": "process_job", "bgColor": "var(--node-bg-listen)", "borderColor": "var(--node-border-listen)", "borderWidth": 3, "fontColor": "var(--node-text-color)"}, "opacity": 1.0, "glowSize": 0, "glowColor": null}, {"id": "query_to_db", "label": "query_to_db", "title": "\n            <div style=\"border-bottom: 1px solid rgba(102,102,102,0.15); padding-bottom: 8px; margin-bottom: 10px;\">\n                <div style=\"font-size: 13px; font-weight: 700; color: #333333; margin-bottom: 6px;\">query_to_db</div>\n                <span style=\"display: inline-block; background: #333333; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;\">listen</span>\n            </div>\n        \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 3px; font-weight: 600;\">Condition</div>\n                    <span style=\"display: inline-block; background: rgba(102,102,102,0.12); color: #666666; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: 700;\">OR</span>\n                </div>\n            \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 4px; font-weight: 600;\">Triggers</div>\n                    <ul style=\"list-style: none; padding: 0; margin: 0;\"><li style=\"margin: 3px 0;\"><code style=\"background: rgba(102,102,102,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #333333; border: 1px solid rgba(102,102,102,0.12);\">extract_metadata</code></li></ul>\n                </div>\n            ", "shape": "custom", "size": 30, "level": 3, "nodeStyle": {"name": "query_to_db", "bgColor": "var(--node-bg-listen)", "borderColor": "var(--node-border-listen)", "borderWidth": 3, "fontColor": "var(--node-text-color)"}, "opacity": 1.0, "glowSize": 0, "glowColor": null}, {"id": "render_and_export_report", "label": "render_and_export_report", "title": "\n            <div style=\"border-bottom: 1px solid rgba(102,102,102,0.15); padding-bottom: 8px; margin-bottom: 10px;\">\n                <div style=\"font-size: 13px; font-weight: 700; color: #333333; margin-bottom: 6px;\">render_and_export_report</div>\n                <span style=\"display: inline-block; background: #333333; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;\">listen</span>\n            </div>\n        \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 3px; font-weight: 600;\">Condition</div>\n                    <span style=\"display: inline-block; background: rgba(102,102,102,0.12); color: #666666; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: 700;\">OR</span>\n                </div>\n            \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 4px; font-weight: 600;\">Triggers</div>\n                    <ul style=\"list-style: none; padding: 0; margin: 0;\"><li style=\"margin: 3px 0;\"><code style=\"background: rgba(102,102,102,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #333333; border: 1px solid rgba(102,102,102,0.12);\">process_cv</code></li><li style=\"margin: 3px 0;\"><code style=\"background: rgba(102,102,102,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #333333; border: 1px solid rgba(102,102,102,0.12);\">process_job</code></li></ul>\n                </div>\n            ", "shape": "custom", "size": 30, "level": 6, "nodeStyle": {"name": "render_and_export_report", "bgColor": "var(--node-bg-listen)", "borderColor": "var(--node-border-listen)", "borderWidth": 3, "fontColor": "var(--node-text-color)"}, "opacity": 1.0, "glowSize": 0, "glowColor": null}, {"id": "route_by_type_1", "label": "route_by_type_1", "title": "\n            <div style=\"border-bottom: 1px solid rgba(102,102,102,0.15); padding-bottom: 8px; margin-bottom: 10px;\">\n                <div style=\"font-size: 13px; font-weight: 700; color: #333333; margin-bottom: 6px;\">route_by_type_1</div>\n                <span style=\"display: inline-block; background: #FF5A50; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;\">router</span>\n            </div>\n        \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 3px; font-weight: 600;\">Condition</div>\n                    <span style=\"display: inline-block; background: rgba(102,102,102,0.12); color: #666666; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: 700;\">OR</span>\n                </div>\n            \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 4px; font-weight: 600;\">Triggers</div>\n                    <ul style=\"list-style: none; padding: 0; margin: 0;\"><li style=\"margin: 3px 0;\"><code style=\"background: rgba(102,102,102,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #333333; border: 1px solid rgba(102,102,102,0.12);\">classify_input</code></li></ul>\n                </div>\n            \n                <div>\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 4px; font-weight: 600;\">Router Paths</div>\n                    <ul style=\"list-style: none; padding: 0; margin: 0;\"><li style=\"margin: 3px 0;\"><code style=\"background: rgba(255,90,80,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #FF5A50; border: 1px solid rgba(255,90,80,0.2); font-weight: 600;\">route_other</code></li><li style=\"margin: 3px 0;\"><code style=\"background: rgba(255,90,80,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #FF5A50; border: 1px solid rgba(255,90,80,0.2); font-weight: 600;\">cv_or_job</code></li></ul>\n                </div>\n            ", "shape": "custom", "size": 30, "level": 1, "nodeStyle": {"name": "route_by_type_1", "bgColor": "var(--node-bg-router)", "borderColor": "#FF5A50", "borderWidth": 3, "fontColor": "var(--node-text-color)"}, "opacity": 1.0, "glowSize": 0, "glowColor": null}, {"id": "route_by_type_2", "label": "route_by_type_2", "title": "\n            <div style=\"border-bottom: 1px solid rgba(102,102,102,0.15); padding-bottom: 8px; margin-bottom: 10px;\">\n                <div style=\"font-size: 13px; font-weight: 700; color: #333333; margin-bottom: 6px;\">route_by_type_2</div>\n                <span style=\"display: inline-block; background: #FF5A50; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;\">router</span>\n            </div>\n        \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 3px; font-weight: 600;\">Condition</div>\n                    <span style=\"display: inline-block; background: rgba(102,102,102,0.12); color: #666666; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: 700;\">OR</span>\n                </div>\n            \n                <div style=\"margin-bottom: 8px;\">\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 4px; font-weight: 600;\">Triggers</div>\n                    <ul style=\"list-style: none; padding: 0; margin: 0;\"><li style=\"margin: 3px 0;\"><code style=\"background: rgba(102,102,102,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #333333; border: 1px solid rgba(102,102,102,0.12);\">query_to_db</code></li></ul>\n                </div>\n            \n                <div>\n                    <div style=\"font-size: 10px; text-transform: uppercase; color: #666666; letter-spacing: 0.5px; margin-bottom: 4px; font-weight: 600;\">Router Paths</div>\n                    <ul style=\"list-style: none; padding: 0; margin: 0;\"><li style=\"margin: 3px 0;\"><code style=\"background: rgba(255,90,80,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #FF5A50; border: 1px solid rgba(255,90,80,0.2); font-weight: 600;\">route_job</code></li><li style=\"margin: 3px 0;\"><code style=\"background: rgba(255,90,80,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: #FF5A50; border: 1px solid rgba(255,90,80,0.2); font-weight: 600;\">route_cv</code></li></ul>\n                </div>\n            ", "shape": "custom", "size": 30, "level": 4, "nodeStyle": {"name": "route_by_type_2", "bgColor": "var(--node-bg-router)", "borderColor": "#FF5A50", "borderWidth": 3, "fontColor": "var(--node-text-color)"}, "opacity": 1.0, "glowSize": 0, "glowColor": null}]);
      this.edges = new vis.DataSet([{"from": "extract_metadata", "to": "query_to_db", "label": "OR", "arrows": "to", "width": 2, "selectionWidth": 0, "color": {"color": "#666666", "highlight": "#666666"}}, {"from": "process_cv", "to": "render_and_export_report", "label": "OR", "arrows": "to", "width": 2, "selectionWidth": 0, "color": {"color": "#666666", "highlight": "#666666"}}, {"from": "process_job", "to": "render_and_export_report", "label": "OR", "arrows": "to", "width": 2, "selectionWidth": 0, "color": {"color": "#666666", "highlight": "#666666"}}, {"from": "classify_input", "to": "route_by_type_1", "label": "OR", "arrows": "to", "width": 2, "selectionWidth": 0, "color": {"color": "#666666", "highlight": "#666666"}}, {"from": "query_to_db", "to": "route_by_type_2", "label": "OR", "arrows": "to", "width": 2, "selectionWidth": 0, "color": {"color": "#666666", "highlight": "#666666"}}, {"from": "route_by_type_1", "to": "handle_other", "label": "route_other", "arrows": "to", "width": 2, "selectionWidth": 0, "color": {"color": "#FF5A50", "highlight": "#FF5A50"}, "dashes": [15, 10]}, {"from": "route_by_type_1", "to": "extract_metadata", "label": "cv_or_job", "arrows": "to", "width": 2, "selectionWidth": 0, "color": {"color": "#FF5A50", "highlight": "#FF5A50"}, "dashes": [15, 10]}, {"from": "route_by_type_2", "to": "process_job", "label": "route_job", "arrows": "to", "width": 2, "selectionWidth": 0, "color": {"color": "#FF5A50", "highlight": "#FF5A50"}, "dashes": [15, 10]}, {"from": "route_by_type_2", "to": "process_cv", "label": "route_cv", "arrows": "to", "width": 2, "selectionWidth": 0, "color": {"color": "#FF5A50", "highlight": "#FF5A50"}, "dashes": [15, 10]}]);
  ...
  setupExportControls() {
    ...
    //   const dagData = {"nodes": {"classify_input": {"type": "start", "method_signature": {"operationId": "classify_input", "parameters": {}, "returns": {"type": "null"}}, "source_code": "    @start()\n    def classify_input(self) -> None:\n        result = ClassificationCrew(\n            verbose=self._verbose,\n            guardrail_max_retries=self._guardrail_max_retries,\n        ).crew().kickoff(inputs={\n            \"user_input\": self.state.raw_input,\n            \"output_options\": \"/\".join(DocumentType),\n        })\n        self.state.input_type = result.raw\n", "source_lines": ["    @start()\n", "    def classify_input(self) -> None:\n", "        result = ClassificationCrew(\n", "            verbose=self._verbose,\n", "            guardrail_max_retries=self._guardrail_max_retries,\n", "        ).crew().kickoff(inputs={\n", "            \"user_input\": self.state.raw_input,\n", "            \"output_options\": \"/\".join(DocumentType),\n", "        })\n", "        self.state.input_type = result.raw\n"], "source_start_line": 42, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "extract_metadata": {"type": "listen", "trigger_methods": ["cv_or_job"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "extract_metadata", "parameters": {}, "returns": {}}, "source_code": "    @listen(\"cv_or_job\")\n    def extract_metadata(self) -> Any:\n        if self.state.input_type == DocumentType.CV:\n            # Extract cv metadata\n            metadata = CVMetadataExtractorCrew(\n                guardrail_max_retries=self._guardrail_max_retries,\n                verbose=self._verbose,\n                human_input=False,\n            ).crew().kickoff(inputs={\n                \"content\": self.state.raw_input,\n                \"educationlevel_options\": \"/\".join(EducationLevel),\n                \"experiencelevel_options\": \"/\".join(ExperienceLevel),\n            })\n        else:\n            # Extract job metadata\n            metadata = JobMetadataExtractorCrew(\n                guardrail_max_retries=self._guardrail_max_retries,\n                verbose=self._verbose,\n                human_input=False,\n            ).crew().kickoff(inputs={\n                \"content\": self.state.raw_input,\n                \"employmenttype_options\": \"/\".join(EmploymentType),\n                \"experiencelevel_options\": \"/\".join(ExperienceLevel),\n            })\n        self.state.metadata = json.loads(metadata.raw)\n", "source_lines": ["    @listen(\"cv_or_job\")\n", "    def extract_metadata(self) -> Any:\n", "        if self.state.input_type == DocumentType.CV:\n", "            # Extract cv metadata\n", "            metadata = CVMetadataExtractorCrew(\n", "                guardrail_max_retries=self._guardrail_max_retries,\n", "                verbose=self._verbose,\n", "                human_input=False,\n", "            ).crew().kickoff(inputs={\n", "                \"content\": self.state.raw_input,\n", "                \"educationlevel_options\": \"/\".join(EducationLevel),\n", "                \"experiencelevel_options\": \"/\".join(ExperienceLevel),\n", "            })\n", "        else:\n", "            # Extract job metadata\n", "            metadata = JobMetadataExtractorCrew(\n", "                guardrail_max_retries=self._guardrail_max_retries,\n", "                verbose=self._verbose,\n", "                human_input=False,\n", "            ).crew().kickoff(inputs={\n", "                \"content\": self.state.raw_input,\n", "                \"employmenttype_options\": \"/\".join(EmploymentType),\n", "                \"experiencelevel_options\": \"/\".join(ExperienceLevel),\n", "            })\n", "        self.state.metadata = json.loads(metadata.raw)\n"], "source_start_line": 60, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "handle_other": {"type": "listen", "trigger_methods": ["route_other"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "handle_other", "parameters": {}, "returns": {"type": "null"}}, "source_code": "    @listen(\"route_other\")\n    def handle_other(self) -> None:\n        msg = f\"Invalid document type. Expected '{DocumentType.CV}' or '{DocumentType.JOB}'. \" \\\n            \"Please, start a new evaluation.\"\n        logger.warning(msg)\n        return msg\n", "source_lines": ["    @listen(\"route_other\")\n", "    def handle_other(self) -> None:\n", "        msg = f\"Invalid document type. Expected '{DocumentType.CV}' or '{DocumentType.JOB}'. \" \\\n", "            \"Please, start a new evaluation.\"\n", "        logger.warning(msg)\n", "        return msg\n"], "source_start_line": 150, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "process_cv": {"type": "listen", "trigger_methods": ["route_cv"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "process_cv", "parameters": {}, "returns": {"type": "null"}}, "source_code": "    @listen(\"route_cv\")\n    def process_cv(self) -> None:\n        # Send info to CVToJobCrew\n        cv_crew = CVToJobCrew(\n            verbose=self._verbose,\n            guardrail_max_retries=self._guardrail_max_retries,\n        )\n        _ = cv_crew.crew().kickoff(\n            inputs={\"structured_cv\": self.state.metadata,\n                    \"related_jobs\": self.state.related_docs}\n        )\n        self.state.process_crew = cv_crew\n", "source_lines": ["    @listen(\"route_cv\")\n", "    def process_cv(self) -> None:\n", "        # Send info to CVToJobCrew\n", "        cv_crew = CVToJobCrew(\n", "            verbose=self._verbose,\n", "            guardrail_max_retries=self._guardrail_max_retries,\n", "        )\n", "        _ = cv_crew.crew().kickoff(\n", "            inputs={\"structured_cv\": self.state.metadata,\n", "                    \"related_jobs\": self.state.related_docs}\n", "        )\n", "        self.state.process_crew = cv_crew\n"], "source_start_line": 109, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "process_job": {"type": "listen", "trigger_methods": ["route_job"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "process_job", "parameters": {}, "returns": {"type": "null"}}, "source_code": "    @listen(\"route_job\")\n    def process_job(self) -> None:\n        # Send info to JobToCVCrew\n        job_crew = JobToCVCrew(\n            verbose=self._verbose,\n            guardrail_max_retries=self._guardrail_max_retries,\n        )\n        _ = job_crew.crew().kickoff(\n            inputs={\"structured_job\": self.state.metadata,\n                    \"related_cvs\": self.state.related_docs}\n        )\n        self.state.process_crew = job_crew\n", "source_lines": ["    @listen(\"route_job\")\n", "    def process_job(self) -> None:\n", "        # Send info to JobToCVCrew\n", "        job_crew = JobToCVCrew(\n", "            verbose=self._verbose,\n", "            guardrail_max_retries=self._guardrail_max_retries,\n", "        )\n", "        _ = job_crew.crew().kickoff(\n", "            inputs={\"structured_job\": self.state.metadata,\n", "                    \"related_cvs\": self.state.related_docs}\n", "        )\n", "        self.state.process_crew = job_crew\n"], "source_start_line": 122, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "query_to_db": {"type": "listen", "trigger_methods": ["extract_metadata"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "query_to_db", "parameters": {}, "returns": {}}, "source_code": "    @listen(extract_metadata)\n    def query_to_db(self) -> Any:\n        if self.state.input_type == DocumentType.CV:\n            collection_name = \"jobs\"\n        else:\n            collection_name = \"cvs\"\n\n        # Get matches from collection\n        related_docs = query_to_collection(\n            collection_name=collection_name,\n            query_text=self.state.raw_input,\n            country=self.state.metadata.get(\"country\"),\n            top_k=3,\n        )\n        self.state.related_docs = related_docs\n", "source_lines": ["    @listen(extract_metadata)\n", "    def query_to_db(self) -> Any:\n", "        if self.state.input_type == DocumentType.CV:\n", "            collection_name = \"jobs\"\n", "        else:\n", "            collection_name = \"cvs\"\n", "\n", "        # Get matches from collection\n", "        related_docs = query_to_collection(\n", "            collection_name=collection_name,\n", "            query_text=self.state.raw_input,\n", "            country=self.state.metadata.get(\"country\"),\n", "            top_k=3,\n", "        )\n", "        self.state.related_docs = related_docs\n"], "source_start_line": 86, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "render_and_export_report": {"type": "listen", "trigger_condition_type": "OR", "condition_type": "OR", "trigger_condition": {"type": "OR", "conditions": ["process_cv", "process_job"]}, "trigger_methods": ["process_cv", "process_job"], "method_signature": {"operationId": "render_and_export_report", "parameters": {}, "returns": {"type": "string"}}, "source_code": "    @listen(or_(process_cv, process_job))\n    def render_and_export_report(self) -> str:\n        # Markdown Report generation\n        report = render_to_markdown(\n            process_type=self.state.input_type,\n            metadata_dict=self.state.metadata,\n            related_docs=self.state.related_docs,\n            gap_analysis_output=self.state.process_crew.identify_gaps_task().output.json_dict,\n            inverview_questions_output=self.state.process_crew.generate_interview_questions_task().output.json_dict,\n        )\n\n        # Write the file\n        REPORT_OUTPUT_PATH.write_text(report, encoding=\"utf-8\")\n        return report\n", "source_lines": ["    @listen(or_(process_cv, process_job))\n", "    def render_and_export_report(self) -> str:\n", "        # Markdown Report generation\n", "        report = render_to_markdown(\n", "            process_type=self.state.input_type,\n", "            metadata_dict=self.state.metadata,\n", "            related_docs=self.state.related_docs,\n", "            gap_analysis_output=self.state.process_crew.identify_gaps_task().output.json_dict,\n", "            inverview_questions_output=self.state.process_crew.generate_interview_questions_task().output.json_dict,\n", "        )\n", "\n", "        # Write the file\n", "        REPORT_OUTPUT_PATH.write_text(report, encoding=\"utf-8\")\n", "        return report\n"], "source_start_line": 135, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "route_by_type_1": {"type": "router", "is_router": true, "router_paths": ["route_other", "cv_or_job"], "trigger_methods": ["classify_input"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "route_by_type_1", "parameters": {}, "returns": {"type": "string"}}, "source_code": "    @router(classify_input)\n    def route_by_type_1(self) -> str:\n        if (self.state.input_type == DocumentType.CV) or (self.state.input_type == DocumentType.JOB):\n            return \"cv_or_job\"\n        else:\n            return \"route_other\"\n", "source_lines": ["    @router(classify_input)\n", "    def route_by_type_1(self) -> str:\n", "        if (self.state.input_type == DocumentType.CV) or (self.state.input_type == DocumentType.JOB):\n", "            return \"cv_or_job\"\n", "        else:\n", "            return \"route_other\"\n"], "source_start_line": 53, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}, "route_by_type_2": {"type": "router", "is_router": true, "router_paths": ["route_job", "route_cv"], "trigger_methods": ["query_to_db"], "trigger_condition_type": "OR", "condition_type": "OR", "method_signature": {"operationId": "route_by_type_2", "parameters": {}, "returns": {"type": "string"}}, "source_code": "    @router(query_to_db)\n    def route_by_type_2(self) -> str:\n        if self.state.input_type == DocumentType.CV:\n            return \"route_cv\"\n        else:\n            return \"route_job\"\n", "source_lines": ["    @router(query_to_db)\n", "    def route_by_type_2(self) -> str:\n", "        if self.state.input_type == DocumentType.CV:\n", "            return \"route_cv\"\n", "        else:\n", "            return \"route_job\"\n"], "source_start_line": 102, "source_file": "c:\\Users\\tania\\Desktop\\multiagent system arnau\\talent_selection_flow_crewai\\notebooks\\..\\src\\talent_selection_flow\\flow.py", "class_signature": "class TalentSelectionFlow(Flow[TalentState])", "class_line_number": 25, "class_name": "TalentSelectionFlow"}}, "edges": [{"source": "extract_metadata", "target": "query_to_db", "condition_type": "OR", "is_router_path": false}, {"source": "process_cv", "target": "render_and_export_report", "condition_type": "OR", "is_router_path": false}, {"source": "process_job", "target": "render_and_export_report", "condition_type": "OR", "is_router_path": false}, {"source": "classify_input", "target": "route_by_type_1", "condition_type": "OR", "is_router_path": false}, {"source": "query_to_db", "target": "route_by_type_2", "condition_type": "OR", "is_router_path": false}, {"source": "route_by_type_1", "target": "handle_other", "condition_type": null, "is_router_path": true, "router_path_label": "route_other"}, {"source": "route_by_type_1", "target": "extract_metadata", "condition_type": null, "is_router_path": true, "router_path_label": "cv_or_job"}, {"source": "route_by_type_2", "target": "process_job", "condition_type": null, "is_router_path": true, "router_path_label": "route_job"}, {"source": "route_by_type_2", "target": "process_cv", "condition_type": null, "is_router_path": true, "router_path_label": "route_cv"}], "start_methods": ["classify_input"], "router_methods": ["route_by_type_1", "route_by_type_2"]};
